...
t.logger.debug('getMainParams dbo typeof=' + typeof t.dbo).tag(t.log.rte).tag(t.log.lne).tagline();			
	...
	processRoute(req, res){
		...
		t.log.remoteIP.setInput(req.headers['cf-connecting-ip'] || req.headers['x-forwarded-for'] || req.connection.remoteAddress);
		t.logger.debug('route(' + route + ')').tag(t.log.rte).tag(t.log.remoteIP).tag(t.log.act).tag(t.log.lne).tagline();
		switch(route){
