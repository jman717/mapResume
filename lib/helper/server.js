...
class server{
	constructor(jo){
		...
		t.views_pages_dir = jo.views_pages_dir;
		/*
	...
	processRoute(req, res){
		...
		switch(route){
			...
			case t.routes.postTestPromiseFactory:
				...
				new mceo({vars: mp}).process();
				break;
			case t.routes.getRolesGrid:
				mp = t.getMainParams(req, res);
				mp.views_pages_dir = t.views_pages_dir;
t.logger.debug('results=debug 10.00').tag(t.log.rte).tag(t.log.cli).tag(t.log.act).tag(t.log.lne).tagline();				
				var f1 = require(t.factory_dir + '/roles_grid');
				var f1o = new f1();
				var toolkit = new ptf().appender('logging', {"type": "log4js-tagline", "log": t.log, "logger": t.logger})
					...
					.appender('functions', {"flow": 1, "map": ["f1o.init"]})
					.appender('functions', {"flow": 2, "map": ["f1o.init", "f1o.render"]})
					.appender('promises', {"flows":[1, 2]})
					.run();
				break;
